...
//        console.log(hCounts.length);
    ...
    let ret = _.map(fHandler([], lCounts), function (iso) {
      return {
        countryIso: iso,
        countPairsTot: _.chain(lCounts).map(function (cp) {
          return (cp.countryFrom === iso) ? cp.nbPubmedIds : 0;
        }).sum().value(),
        countPairs: _.filter(lCounts, function (cp) {
          return cp.countryFrom === iso
        }),
        countTot: _.find(lCounts, function (cp) {
          return cp.countryFrom === iso
        }).nbPubmedIdTotalFrom
      }
    });
